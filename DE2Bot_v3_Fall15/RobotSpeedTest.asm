...
WaitForFour:
  ...
  ADDI 1
  ISTORE Ptr
  JPOS DontWrite ; If we've overflowed
  JNEG DontWrite
  LOAD Ptr    ; Add one to the pointer
  ADDI 1
  STORE Ptr
  ILOAD Ptr   ; write the carry
  ADDI 1
  ISTORE Ptr
  ...
  LOAD Offset     ; then we should record to the next location
  ADDI 2
  STORE Offset
...
TicksPerFourFeet: DW 1172 ;from 1.04 mm / tick, ~304 mm / foot, 4 feet.
TwoSeventy:       DW 270  ;

Offset:           DW 0
Ticks:            DW 0    ; from here, saves speed information on a clock level
                          ; these values are 32 bit, and requires 2 words to
                          ; store
