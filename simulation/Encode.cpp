...
ByteArray hexDecode(std::string const& encodedData) {
  ...
  size_t decoded = hexDecode(encodedData.c_str(), encodedData.size(), res.ptr(), res.size());
  _unused(decoded);
...
ByteArray base64Decode(std::string const& encodedData) {
  ...
  size_t decoded = base64Decode(encodedData.c_str(), encodedData.size(), res.ptr(), res.size());
  _unused(decoded);
  assert(decoded <= res.size());
  res.resize(decoded);
  return res;
