...
public:
  ...
  struct IntersectResult {
    ...
    bool intersects;
    // Where the intersection is (minimum value if intersection occurs in more than one point.)
    VectorType point;
    // T value where intersection occurs, 0 is min, 1 is max
    T t;
    // Whether or not the two lines, if they were infinite lines, are the exact same line
    bool coincides;
    // Whether or not the intersection is a glancing one, meaning the other line isn't actually skewered, it's just barely touching
    // Coincidental lines are always glancing intersections.
    bool glances;
  };
  ...
  template <size_t P=N>
  typename std::enable_if<P == 2 && N == P, IntersectResult>::type intersection(Line const& line2, bool infinite = false) const {
    Line l1 = *this;
    Line l2 = line2;
    // Warning to others, do not make the lines positive, because points of intersection for coincidental lines are determined by the first point
    // And makePositive() changes the order of points.  This causes headaches later on
    //l1.makePositive();
    //l2.makePositive();
    VectorType a = l1.min();
    VectorType b = l1.max();
  ...
  template <size_t P=N>
  typename std::enable_if<P == 2 && N == P, void>::type rotate(T angle, VectorType const& rotationCenter = VectorType()) {
